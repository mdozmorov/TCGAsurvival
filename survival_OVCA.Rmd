...
for (cancer_type in cancer_RNASeq2) {
  ...
  clin_full <- clin_full[rownames(clin_full) %in% expr$AffyID, ]
  clin_full <- clin_full[match(expr$AffyID, rownames(clin_full)), ]
  all.equal(expr$AffyID, rownames(clin_full))
  
  # Prepare extra clinical annotation
  clin_extra <- read.table("data.TCGA/OvarianCancerSubtypes/data/23257362/TCGA_489_UE.k4.txt", sep = "\t", header = TRUE)
