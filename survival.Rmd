...
system("rm -r res.*") # Remove previous results folders, if any
```
...
```{r eval=FALSE}
# If needed, same the full clinical annotation and the expression matrices in an Excel file
library(MDmisc)
fileName <- paste0("results/", cancer, ".xlsx") # File name to save the results
unlink(fileName) # Delete the old file, if it exists
wb <- openxlsx::createWorkbook(fileName) # Create new worksheet
# Make expression matrix
mtx_expr <- data.frame(ID = mtx$merged.dat[, "bcr"], mtx$merged.dat[ , 4:ncol(mtx$merged.dat)]) # Append IDs
# Make clinical matrix
mtx_clin <- mtx$clinical[rownames(mtx$clinical) %in% mtx_expr$ID, ] # Subset full clinical data to IDs that have expression
mtx_clin <- mtx_clin[match(mtx_expr$ID, rownames(mtx_clin)), ] # Match the order of expression and clinical sample IDs
mtx_clin <- data.frame(ID = rownames(mtx_clin), mtx_clin) # Append IDs
all_equal(mtx_expr$ID, mtx_clin$ID) # Should be TRUE
# Save the results
save_res(mtx_expr, fileName = fileName, wb = wb, sheetName = "Expression")
save_res(mtx_clin, fileName = fileName, wb = wb, sheetName = "Clinical")
```
...
# selected_genes = c("PECAM1", "S1PR1", "SPNS2", "TEK", "TIE1"); cancer = c("PAAD")
# selected_genes = c("hsa.mir.142"); cancer = c("BRCA")
...
if (subcategories_in_all_cancers) {
  cancer <- c("ACC", "BLCA", "BRCA", "CESC", "CHOL", "COAD", "COADREAD", "DLBC", "ESCA", "GBM", "GBMLGG", "LGG", "HNSC", "KICH", "KIPAN", "KIRC", "KIRP", "LIHC", "LUAD", "LUSC", "MESO", "OV", "PAAD", "PCPG", "PRAD", "READ", "SARC", "SKCM", "STAD", "TGCT", "THCA", "THYM", "UCEC", "UCS") 
} else {
  cancer <- cancer # Selected in settings
}
data.type = "RNASeq2"; type = "" 
...
# All cancers with RNASeq2 data
cancer = c("ACC", "BLCA", "BRCA", "CESC", "CHOL", "COAD", "COADREAD", "DLBC", "ESCA", "GBM", "HNSC", "KICH", "KIPAN", "KIRC", "KIRP", "LIHC", "LUAD", "LUSC", "MESO", "OV", "PAAD", "PCPG", "PRAD", "READ", "SARC", "SKCM", "STAD", "TGCT", "THCA", "THYM", "UCEC", "UCS") # "GBMLGG", "LGG", 
data.type = "RNASeq2"; type = "" 
# Process each cancer
for (cancer_type in cancer) {
  print(paste0("Processing cancer ", cancer_type))
