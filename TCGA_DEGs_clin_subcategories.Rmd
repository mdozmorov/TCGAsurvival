...
min_samples <- 15
# Functional category, from http://amp.pharm.mssm.edu/Enrichr/#stats
enrichr_category <- c("Chromosome_Location", "KEGG_2016")
...
# Enrichment cutoffs
fdr.cutoff     <- 0.3
up_dn_separate <- FALSE # Whether to run functional separately on up- and downregulated genes. FALSE - do not distinguish directionality
top_X          <- 10 # How many top significant differentially expressed genes/pathways to output

# Filename to same the results
fileNameRes <- paste0("results/", cancer, "_DEGs_clin_subcategories.xlsx")
```
...
Results are stored in the Excel file `r fileNameRes`. Sheets are named by numbers. Each results output has a header explaining what the results are.
  ...
  for (i in 1:ncol(annotations_combn)) {
    ...
    if (nrow(degs) > 0) {
      ...
      if (full_degs_output) { # Save the full DEG list?
        degs_to_save <- data.frame(Gene = rownames(degs), topTable(fit2, coef = 1, number = Inf)) # Append gene names to the DEG list
      } else {
      ...
      if (up_dn_separate) {
        ...
        print(paste0(enrichr_category, " analysis run on ", length(up.gene), " upregulated and ", length(dn.genes), " downregulated genes."))
        res.kegg <- save_enrichr(up.genes = up.genes, dn.genes = dn.genes, databases = enrichr_category, fdr.cutoff = fdr.cutoff)
      } else {
        # Analyze up- and downregulated genes together
        print(paste0(enrichr_category, " analysis run on ", length(unique(c(up.genes, dn.genes))), " genes without distinguishing them by directionality."))
        res.kegg <- save_enrichr(up.genes = unique(c(up.genes, dn.genes)), databases = enrichr_category, fdr.cutoff = fdr.cutoff)
      }
      ...
      if (nrow(res.kegg) > 0) {
        ...
        header[1, 1] <- paste0(nrow(res.kegg), " enrichments in ", enrichr_category, " in ", cancer, ", between groups \'", annotation, ":", group2, "\' vs. \'", annotation, ":", group1, "\'") # Construct header
