...
- Select "RSEM-barplot" option on the sidebar. Read more about RSEM selected_gene expression measures at [https://deweylab.github.io/RSEM/](https://deweylab.github.io/RSEM/)
...
cells <- read.table("data.TCGA/CCLE_Cell_lines_annotations_20181226.txt", sep = "\t", header = TRUE, fill = TRUE)
...
The table is sorted by "p_value" column, from most to least significant. Positive/negative log2 Fold change indicate the gene is up/down in cancer samples, respectively.
...
t.table <- t.table[ order(as.numeric(t.table$p_value), decreasing = FALSE), ]
