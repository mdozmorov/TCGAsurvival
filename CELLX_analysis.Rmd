...
Column names: "Cancer.Name", "Acronym" - cancer type, "Fold_change" - fold change (positive/negative - upregulated/downregulated in cancer), "p_value", "t_statistic" - results of Welch two-sample t-test, "Normal", "Cancer" - number of samples<!--, "Mean_expression_normal", "Mean_expression_cancer" - log2-transformed RSEM expression-->.
...
for (i in 1:length(cancer)) {
    ...
    t.table[i, 7:9] <- c(round(res$statistic, 3), round(res$parameter, 3), res$p.value, 6)
...
t.table <- t.table[ !is.na(t.table$Normal), ] # Excluded cancers without normal samples
...
t.table <- t.table[, c("Cancer.Name", "Acronym", "Fold_change", "p_value", "Normal", "Cancer", "t_statistic")] # , "Mean_expression_normal", "Mean_expression_cancer"
# DT::datatable(t.table, width = 800)
# Save the full results
write.csv(t.table, "CELLX_analysis.csv")
# Visualize the most important
t.table$p_value <- formatC(as.numeric(t.table$p_value), format = "e", digits = 2)
kable(t.table[ t.table$Normal > min_number_of_normal, ])
```    
